var numDigits = 4;
var sheetSuffix = " encrypted";
var plainSheetSuffix = " decrypted";

function onOpen() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var menuEntries = [ {name: "Encrypt sheet", functionName: "encryptSheet"},
                      {name: "Decrypt sheet", functionName: "decryptSheet"} ];
  ss.addMenu("Encryption", menuEntries);
}

function dialogClosed(key, hint){
  doEncryption(key, hint);
}

function encryptSheet(){
  var ui = SpreadsheetApp.getUi();  
  var html = HtmlService.createHtmlOutput(getHtmlForDialog()).setWidth(380).setHeight(150);
  ui.showModalDialog(html, "Enter Key");  
  return;
}

function doEncryption(key, hint){
  var ss = SpreadsheetApp.getActive();
  var plainSheet = ss.getActiveSheet();
  var sheetName = plainSheet.getName();
  var newSheetName = sheetName + sheetSuffix;
  
  var encSheet = ss.getSheetByName(newSheetName);
  if (encSheet != null) ss.deleteSheet(encSheet);
  encSheet = ss.insertSheet();
  encSheet.setName(newSheetName);

  var s = "";
  var range = plainSheet.getDataRange();
  var values = range.getValues();
  for (i = 0; i < values.length; i++) {
    var row = values[i];
    for (j = 0; j < values[i].length; j++) {
      var val = values[i][j];
      s += pad(i + 1, numDigits) + pad(j + 1, numDigits) + pad(val.toString().length, numDigits) + val;
    }
  }  

  var cipherText = CryptoJS.Rabbit.encrypt(s, key).toString();
  encSheet.getRange(3,1).setValue(cipherText);
  
  // save the formatting
  var nCols = range.getLastColumn();
  var nRows = range.getLastRow();
  range.copyFormatToRange(encSheet, range.getColumn(), nCols, 4, nRows);
  encSheet.getRange(1,2).setValue(nRows);
  encSheet.getRange(1,3).setValue(nCols);
  for(var i=1; i<=nCols; i++){
    encSheet.getRange(2, i).setValue(plainSheet.getColumnWidth(i));
  }
  if(hint.length > 0) encSheet.getRange(1,1).setValue("Hint: " + hint);
}
...
function decryptSheet(){
  ...
  var ss = SpreadsheetApp.getActive();
  var encSheet = ss.getActiveSheet();
  var encSheetName = encSheet.getName();
  var plainSheetName;
  if (encSheetName.indexOf(sheetSuffix) > -1)
    plainSheetName = encSheetName.substr(0, encSheetName.length - sheetSuffix.length);
  else
    plainSheetName = encSheetName + plainSheetSuffix;
  
  var plainSheet = ss.getSheetByName(plainSheetName);
  if (plainSheet != null) ss.deleteSheet(plainSheet);
  plainSheet = ss.insertSheet();
  plainSheet.setName(plainSheetName);
  
  // transfer formatting
  var nRows= encSheet.getRange(1, 2).getValue();
  var nCols = encSheet.getRange(1, 3).getValue();
  var formatRange = encSheet.getRange(4, 1, nRows, nCols);
  formatRange.copyFormatToRange(plainSheet, 1, nCols, 1, nRows);
  for(var i=1; i<=nCols; i++){
    plainSheet.setColumnWidth(i, encSheet.getRange(2, i).getValue());
  }
  
  try {
    var p = CryptoJS.Rabbit.decrypt(encSheet.getRange(3,1).getValue(), key).toString(CryptoJS.enc.Utf8);
  
    var index = 0;
    while (index < p.length){
      var row = +(p.substr(index, numDigits));
      var col = +(p.substr(index + numDigits, numDigits));
      var len = +(p.substr(index + 2 * numDigits, numDigits));
      if(len > 0) plainSheet.getRange(row, col).setValue(p.substr(index + 3 * numDigits, len));
      index += (3 * numDigits) + len;    
    }
  }
  catch(e) {
    ui.alert("Decryption failed. Did you enter the correct password?");
  }
}
...
function getHtmlForDialog(){
  return "<html><style>.column{float:left;width:50%}</style><div style='background-color:lightblue;font-family:verdana;padding:10px;overflow:auto'> " +
	"<span class=column>Enter key:</span><input type=text id='txtKey' style='float:right'><br/><br/>" +
	"<span class=column>Enter hint (optional):</span><input type=text id='txtHint' style='float:right'><br/><br/>" +
    "<div style='float:right'>" +
    "<input type=button value='Cancel' onclick='google.script.host.close();'>&nbsp;<input type=button value='OK' onclick=OKClicked()>" +
    "</div></div><script>" +
	"function OKClicked(){" +
		"var theKey = document.getElementById('txtKey').value;" +
        "if(theKey.length == 0){alert('You must enter a key'); return;}" +
        "var hint = document.getElementById('txtHint').value;" +
		"google.script.run.withSuccessHandler(onSuccess).dialogClosed(theKey, hint);" +
    "}" +
    "function onSuccess(){" +
